class MyPagesController < ApplicationController
  def show
    @beans = Bean.where(user_id: current_user.id).page(params[:page]).order(created_at: :desc)
    @recipes = Recipe.where(user_id: current_user.id).page(params[:page]).order(created_at: :desc)
