class Recipe < ApplicationRecord
  ...
  has_many :recipe_liked_users, through: :recipe_likes, source: :recipe
	...
	scope :search_hot_ice, -> (hot_ice) {where('hot_ice LIKE ?', "#{hot_ice}") if hot_ice.present?}
	scope :search_extraction, -> (extraction) {where('extraction LIKE ?', "#{extraction}") if extraction.present?}
